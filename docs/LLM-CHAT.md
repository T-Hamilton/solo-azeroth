...
| This core's `Player::IsInChannel` compares channel IDs, so anyone in a General was "in" every General. Remote bots were taken for members of your channel, never joined, and the channel silently rejected their lines while the module counted them delivered. | In Tirisfal 97 lines "delivered", 8 visible; Silvermoon and Orgrimmar looked dead. | Audience checks use the exact instance (same id and the one for the player's own zone); bots are always joined to the channel at delivery. |
